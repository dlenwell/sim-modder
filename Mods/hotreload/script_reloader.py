...
import linecache
import os
from constants import LOCAL_MODS
...
def hot_reload(*args, _connection=None):
    ...
    if not args:
        ...
        if module is None:
            ...
            output('Could not reload built-in module: {}'
                   .format(module))
        else:
...
def _reload(module, filename, mod, output):
    ...
    code = None
    try:
        code = compile(open(os.path.join(LOCAL_MODS, mod, file)).read(), mod, 'exec')
        output('Reloaded {}'.format(filename))
    except:
        output('Exception encountered while reloading from {}'.format(LOCAL_MODS, mod, file))
    if code is not None:
        tmpns = modns.copy()
        modns.clear()
